...
uint32_t PWM_Freq = 50;     // freq do PWM
uint32_t PWM_Resol = 200;    // resolucao do PWM (qtde passos do PWM)
...
int main(void)
{
  ...
  while (1)
  {
	...
	GPIO_WriteBit(GPIOB, GPIO_Pin_14, Bit_RESET); // apaga o LED PC14
	...
	// aumenta a intensisdade de 0 para 100 em 2 segundos
	if (millis()>=prx_PB7)
	{
		brilho = brilho + inc;         // incrementa/decrementa brilho
		if (brilho > pwm_lim) { // se brilho > PWM_Resol, limita
